...
- **Portfolio, not production**: a small, personal, from-scratch project. Every
  ...
  own runs. Never imply employer (Intuit) production experience.
...
descriptions in `docs/` and `~/projects/Resume/sample_jd`).
...
- Public-repo safe: no real user data, no employer (Intuit) data, no PII.
